"""Dump native UEFN 42.20 ToolsetRegistry schemas from the running editor.

Run this file inside UEFN's Python environment, for example:

    py "C:/path/to/dump_native_toolsets.py"

The generated files are written to ``<project>/Saved/ToolsetDumps/UEFN-42.20``.
...
# Candidate modules from the 42.10 extraction. Each is probed independently;
# 42.20 may add, remove, or rename modules, so failures are recorded in the
# module-load report and reflection still discovers any other loaded classes.
EPIC_TOOLSET_MODULES = (
    "AIAssistant",
    "AIModuleToolset",
...
def _load_epic_toolset_modules() -> dict[str, object]:
    ...
    for module_name in EPIC_TOOLSET_MODULES:
...
def _build_agent_ready_markdown(agent_index: dict[str, object]) -> str:
    ...
    lines = [
        f"# Agent-ready native UEFN tools — {UEFN_VERSION}",
        "",
...
def _build_index_markdown(
    ...
    lines = [
        ...
        "",
        f"The output is written under the project's `Saved/ToolsetDumps/UEFN-{UEFN_VERSION}`:",
        "",
...
def dump_native_toolsets() -> dict[str, object]:
    ...
    output_dir = Path(saved_dir) / "ToolsetDumps" / f"UEFN-{UEFN_VERSION}"
